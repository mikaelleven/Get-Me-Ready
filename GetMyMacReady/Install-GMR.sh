#!/usr/bin/env bash
# Downloads the macOS GetMeReady registry without requiring Git.
set -euo pipefail

repository_archive_url='https://github.com/mikaelleven/Get-Me-Ready/archive/refs/heads/master.tar.gz'
install_path="${HOME}/Library/Application Support/GetMeReady"
...
source_path="$archive_root/GetMyMacReady"

if [[ -z "$archive_root" || ! -d "$source_path" ]]; then
    printf '%s\n' 'The downloaded archive did not contain the macOS registry.' >&2
    exit 1
fi

mkdir -p "$(dirname "$install_path")"
cp -R "$source_path" "$install_path"
printf 'GetMeReady macOS registry was installed to %s\n' "$install_path"
printf '%s\n' 'A macOS GMR runner is not available yet, so no setup actions were executed.'
